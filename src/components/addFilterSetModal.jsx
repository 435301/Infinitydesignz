...
import { addFilterSet } from '../redux/actions/filterSetAction';

const AddFilterSetModal = ({ show, onClose }) => {
  ...
  const validate = () => {
    const newErrors = {};
    if (!selectedFilterTypeId) newErrors.menu = 'Filter Type is required';
    filterTypesInput.forEach((field, index) => {
      if (!field.title.trim()) newErrors[`title-${index}`] = 'Title is required';
      if (!field.priority || isNaN(field.priority)) newErrors[`priority-${index}`] = 'Priority must be a number';
    });
    setErrors(newErrors);
  ...
  const handleAddField = () => {
    setFilterTypesInput([...filterTypesInput, { title: '', priority: '' }]);
  };
  ...
  const handleRemoveField = (index) => {
    ...
    delete updatedErrors[`priority-${index}`];
    setErrors(updatedErrors);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      for (let item of filterTypesInput) {
        ...
        await dispatch(addFilterSet(payload));
      }
                ...
                <div className="mb-3 d-flex align-items-start gap-2" key={index}>
                  ...
                  <div className="d-flex flex-column">
                    {filterTypesInput.length > 1 && (
                      <button type="button" className="btn btn-outline-danger mb-1" onClick={() => handleRemoveField(index)}>
                        <BsDashCircle />
                      </button>
                    )}
...
export default AddFilterSetModal;
