...
import { editSliders } from '../redux/actions/slidersAction';

const EditSliderModal = ({ isOpen, onClose, slider }) => {
    ...
    useEffect(() => {
        if (slider) {
            ...
            setLink(slider.link || '');
            setPriority(slider.priority || '');
            setStatus(slider.status || false);
            setSliderPreview(`${BASE_URL}${slider.image_url}`);
            setSliderImage(null);
        }
    }, [slider]);
    ...
    const validateForm = () => {
        ...
        if (!sliderPreview && !sliderImage) errors.image = 'Slider image is required.';
        // if (!priority.trim()) {
...
const handleSubmit = async (e) => {
   ...
   const formData = new FormData();
    ...
    formData.append('priority', String(priority)); // Convert to string
    formData.append('status', String(status)); // Convert to string
    ...
    for (let pair of formData.entries()) {
     ...
     console.log('id1', slider.id)
if (!slider || !slider.id) {
  alert("Slider data not loaded.");
  return;
}

   dispatch(editSliders(slider.id, formData));
    onClose(); 
};
